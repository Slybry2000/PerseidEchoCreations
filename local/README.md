...
## The second layer: `/workflows/`

Added 2026-07-30. Five problem-shaped pages plus a hub, generated by the same script from `_build/workflows.json`.

```
/workflows/                                the hub
/workflows/quote-follow-up/
/workflows/field-paperwork-to-invoice/
/workflows/order-intake-from-email/
/workflows/appointment-reminders/
/workflows/client-intake-and-documents/
```

**Why these are not tool guides.** Owners do not search "business process automation", they search "automate quote follow up". But that SERP was checked on 2026-07-30 and it belongs to Salesforce, NetSuite, SPS Commerce and a wall of vendor blogs — head-on competition a one-operator shop loses, and worse, it attracts people shopping for *software* rather than for a builder.

What is winnable is the question those searchers hit immediately afterwards and nobody neutral answers: **buy a tool, or have something built?** Every page is that decision, argued honestly, with a "buy the tool" column that is real. The appointment-reminders page tells the reader to go and check their booking system's settings first and probably not hire anyone.

**That is the load-bearing part.** If a future edit makes PEC the right answer on every page, these stop being useful and start being adverts, and they will convert worse rather than better. The `_note` at the top of `workflows.json` says the same thing; read it before editing.

Each page carries `Article` + `BreadcrumbList` + `FAQPage`, uses the `ind-bench` plate rather than `ind-street` so the two layers feel distinct, and cross-links both ways with the city pages via the `cities` array on each workflow.

Content rules specific to this layer:

- **No invented statistics.** Every page asks the reader to work out their own number from their own business. There is no "companies save 40% on average" anywhere, because there is no source for it.
- **"Do not build this yet" gets its own block** on every page, not a footnote.
- The arithmetic sections must stay reader-supplied. The moment one of them quotes a benchmark, it needs a citation or it comes out.
